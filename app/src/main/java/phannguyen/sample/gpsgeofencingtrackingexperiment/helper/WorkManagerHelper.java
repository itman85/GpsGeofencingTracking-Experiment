...
import phannguyen.sample.gpsgeofencingtrackingexperiment.worker.ActivityTrackingIntervalWorker;
...
import static phannguyen.sample.gpsgeofencingtrackingexperiment.utils.Constant.ACTIVITY_TRACKING_INTERVAL_WORKER_TAG;
import static phannguyen.sample.gpsgeofencingtrackingexperiment.utils.Constant.ACTIVITY_TRACKING_INTERVAL_WORKER_UNIQUE_NAME;
...
public class WorkManagerHelper {
    ...
    public static void startActivityTriggerWorkerOnetimeRequest(Context context,int delayInSecond, int workPolicyVal){
        ...
        OneTimeWorkRequest locationIntervalWork=
                new OneTimeWorkRequest.Builder(ActivityTrackingIntervalWorker.class)
                        .setInitialDelay(delayInSecond, TimeUnit.SECONDS)
                        .addTag(ACTIVITY_TRACKING_INTERVAL_WORKER_TAG)// Use this when you want to add initial delay or schedule initial work to `OneTimeWorkRequest` e.g. setInitialDelay(2, TimeUnit.HOURS)
                        .build();
        WorkManager.getInstance(context).enqueueUniqueWork(ACTIVITY_TRACKING_INTERVAL_WORKER_UNIQUE_NAME,workPolicy,locationIntervalWork);
    }
    ...
    public static void cancelActivityTriggerWorkerOnetimeRequest(Context context){
        FileLogs.writeLog(context,"WorkManagerHelper","I","cancel Activity TriggerWorker Onetime Request");
        WorkManager.getInstance(context).cancelUniqueWork(ACTIVITY_TRACKING_INTERVAL_WORKER_UNIQUE_NAME);
    }
}
